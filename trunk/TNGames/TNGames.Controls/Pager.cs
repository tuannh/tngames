    ...
    public class Pager : WebControl, IPostBackEventHandler, INamingContainer
    {
        ...
        private string RenderGoToScript()
        {
            ...
            sb.AppendFormat("gotoImg.src = gotoElem.style.display == 'none' ? '{0}' : '{1}';",
                                    Page.ClientScript.GetWebResourceUrl(this.GetType(), "ASPnetPager.Images.arr_right.gif"),
                                    Page.ClientScript.GetWebResourceUrl(this.GetType(), "ASPnetPager.Images.arr_left.gif")
                                    );
            sb.Append("}");
